...
public class Foreman.Widgets.Dialogs.ServerAddressDialog : Granite.Dialog {

    public uint port { get; construct set; }

    public ServerAddressDialog (Foreman.Windows.MainWindow main_window, uint port) {
        Object (
            deletable: false,
            resizable: false,
            title: _("Connecting to Your Server"),
    ...
    construct {
        ...
        var header_image = new Gtk.Image.from_icon_name ("preferences-system-network", Gtk.IconSize.DIALOG);

        var header_title = new Gtk.Label (_("Connecting to Your Server")) {
        ...
        int num_chars = 50;
        var local_connection_header_label = new Granite.HeaderLabel (_("Local Connections"));
        ...
        // XXX: Temporary
        debug ("Private IP: %s", Foreman.Utils.NetUtils.get_private_ip_addr ());
        debug ("Public IP: %s", Foreman.Utils.NetUtils.get_public_ip_addr ());
    }
